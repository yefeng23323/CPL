...
from .query_support import QuerySupportDetectorCPL
...
class CPL(QuerySupportDetectorCPL, TestMixins):
    """
    Args:
    ...
    def extract_support_feat(self, img):
        ...
        Args:
            img (Tensor): Input images of shape (N, C, H, W).
                Typically these should be mean centered and std scaled.   均值和std缩放
        Returns:
            list[Tensor]: Features of input image, each item with shape
                (N, C, H, W).
        """
        feats = self.backbone(img, use_meta_conv=True) # (b,c,14,14)
    ...
    def forward_model_init(self,
        ...
        roi_feat = self.roi_head.extract_support_feats(feats)  # roi_feat[0] = [15,2048]
        self._forward_saved_support_dict['roi_feats'].extend(roi_feat)
        gt_labels_ = torch.cat(gt_labels)

        if getattr(self.roi_head, 'feats_pooling', False):
            pooling_feats = self.roi_head.feats_pooling(feats[0], support_gt_labels=gt_labels_)
            self._new_forward_saved_support_dict['gt_labels'].extend(gt_labels)
            self._new_forward_saved_support_dict['pooling_feats'].extend([pooling_feats])
        return {'gt_labels': gt_labels, 'roi_feat': roi_feat}

    def model_init(self):
        pass

    def cpl_model_init(self):
        ...
        if getattr(self.roi_head, 'feats_pooling', False):
            pooling_feats = torch.cat(self._new_forward_saved_support_dict['pooling_feats'])
            for k in self.new_inference_support_dict.keys():
                self.new_inference_support_dict[k].clear()
            for class_id in class_ids:             
                self.new_inference_support_dict['pooling_feats'][class_id] = pooling_feats[
                    gt_labels == class_id].mean([0], True)

            pooling_feats = torch.cat(
                [self.new_inference_support_dict['pooling_feats'][class_id] for class_id in class_ids], dim=0)
            self.new_inference_support_dict['pooling_feats'][0] = pooling_feats
            keys = list(self.new_inference_support_dict['pooling_feats'].keys())

            for k in keys:
                if k != 0:
                    self.new_inference_support_dict['pooling_feats'].pop(k)
