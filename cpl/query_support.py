...
class QuerySupportDetectorCPL(BaseDetector):
    ...
    def extract_query_feat(self, img: Tensor) -> List[Tensor]: 
        ...
        # pdb.set_trace()
        # backbone = {'type': 'ResNetWithMetaConv', 'depth': 101, 'num_stages': 3, 'strides': (1, 2, 2), 'dilations': (1, 1, 1), 'out_indices': (0,1,2,), 'frozen_stages': 2, 'norm_cfg': {'type': 'BN', 'requires_grad': False}, 'norm_eval': True, 'style': 'caffe', 'pretrained': None}
        # self.backbone = build_backbone(backbone)
        # feats2 = self.backbone(img)  [[] [] []] 
    ...
    def forward_train(self,
        ...
        query_feats = self.extract_query_feat(query_img)  # [6, 1024, 48, 72]  
        support_feats = self.extract_support_feat(support_img)  # [20, 1024, 14, 14]

        losses = dict()

        
        if self.post_rpn: # post_rpn==True -> roi_head.py train
            proposal_list = None
        if not self.post_rpn:
